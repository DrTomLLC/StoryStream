...
pub(crate) fn row_to_book(row: sqlx::sqlite::SqliteRow) -> Result<Book, AppError> {
    ...
    let id = BookId::from_string(&id_str)
    ...
    let added_date_ms: i64 = row.try_get("added_date")
...
mod tests {
    ...
    async fn test_update_book() {
        ...
        update_book(&pool, &book).await.expect("Failed to update book");

        let retrieved = get_book(&pool, book.id).await.expect("Failed to get updated book");
    ...
    async fn test_delete_book() {
        ...
        create_book(&pool, &book).await.expect("Failed to create book");
        delete_book(&pool, book.id).await.expect("Failed to delete book");
    ...
    async fn test_list_books() {
        ...
        create_book(&pool, &book1).await.expect("Failed to create book 1");
        create_book(&pool, &book2).await.expect("Failed to create book 2");
    ...
    async fn test_get_books_by_author() {
        ...
        let books = get_books_by_author(&pool, "Author A").await.expect("Failed to get books by author");
        assert_eq!(books.len(), 2);
    }
    ...
    async fn test_get_favorite_books() {
        ...
        create_book(&pool, &book1).await.expect("Failed to create favorite book");
        create_book(&pool, &book2).await.expect("Failed to create regular book");

        let favorites = get_favorite_books(&pool).await.expect("Failed to get favorite books");
    ...
    async fn test_get_recently_played_books() {
        ...
        create_book(&pool, &book1).await.expect("Failed to create played book");
        create_book(&pool, &book2).await.expect("Failed to create unplayed book");

        let recent = get_recently_played_books(&pool, 10).await.expect("Failed to get recently played books");
