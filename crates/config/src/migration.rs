...
pub fn migrate_to_latest(config: Config) -> ConfigResult<Config> {  // Remove mut
    ...
    let migrated: Config = toml::from_str(&toml::to_string(&value)?)
...
mod tests {
    ...
    fn test_migrate_same_version() {
        ...
        let migrated = migrate_to_latest(config.clone()).expect("Should migrate");
    ...
    fn test_migrate_newer_version() {
        ...
        let migrated = migrate_to_latest(config.clone()).expect("Should handle newer version");
